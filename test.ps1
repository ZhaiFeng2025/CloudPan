# CloudPan Phase 0 一键测试脚本
# 在 PowerShell 中运行: .\test.ps1
...
$upload = Invoke-RestMethod -Uri "$Server/api/files/upload" -Method Post -Form @{
...
$mkdir = Invoke-RestMethod -Uri "$Server/api/files/mkdir" -Method Post -Body '{"path":"/docs/"}' -ContentType "application/json"
Write-Host "   文件夹: $($mkdir.data.path)" -ForegroundColor Green

# 4. 文件树
Write-Host "4. 文件树..." -ForegroundColor Yellow
$tree = Invoke-RestMethod -Uri "$Server/api/files/tree?sinceVersion=0&limit=10"
...
Invoke-WebRequest -Uri "$Server/api/files/download?path=$encodedPath" -OutFile "$TestDir\download.txt"
...
$search = Invoke-RestMethod -Uri "$Server/api/files/search?q=hello"
