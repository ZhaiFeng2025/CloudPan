...
/// <summary>托盘应用操作协作类（T-109）：打开文件夹/日志目录、开机自启注册表、设置窗口与退出。</summary>
internal sealed class TrayActions
{
    private readonly TrayAppContext _ctx;

    public TrayActions(TrayAppContext ctx)
    {
        _ctx = ctx;
    }
    ...
    public void OpenSettings()
    {
        try
        {
            ...
            ClientConfig cfg = _ctx._config;
            SettingsForm form = new SettingsForm(Program.ServerUrl, Program.SyncRoot, Program.Token,
                cfg.UploadLimitBps, cfg.DownloadLimitBps, cfg.SelectedPaths, () => _ctx._engine.GetDirectoryTreePathsAsync());
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                _ctx._api.SetUploadLimit(cfg.UploadLimitBps);
                _ctx._api.SetDownloadLimit(cfg.DownloadLimitBps);
                _ctx._engine.UpdateSelectedPaths(cfg.SelectedPaths);
