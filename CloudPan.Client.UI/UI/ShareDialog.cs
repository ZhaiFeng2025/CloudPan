...
{
    /// <summary>
    /// T-018：分享对话框。过期时间以 ISO 8601 UTC（"O" 格式 Z 后缀）发送，与服务端 TryParse(InvariantCulture, AdjustToUniversal) 对齐。
    /// </summary>
    public static void Show(IWin32Window owner, SyncEngine engine, FileBrowseItem item, Action<string> addLog)
    {
        ...
        async void OnGenerateClick(object? s, EventArgs e)
        {
            ...
            try
            {
                ...
                var result = await engine.CreateShareAsync(
        ...
        async void OnRevokeClick(object? s, EventArgs e)
        {
            ...
            try
            {
                ...
                bool ok = await engine.RevokeShareAsync(shareId);
