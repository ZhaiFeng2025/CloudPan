...
public class GlobalExceptionMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        catch (Exception ex)
        {
            ...
            // AggregateException 递归展平：所有内部异常写入日志，逐个匹配友好消息
            if (ex is AggregateException aggEx)
            {
                var flattened = aggEx.Flatten();
                foreach (var innerEx in flattened.InnerExceptions)
                {
                    logger?.LogError(innerEx, "AggregateException 内部异常: {Path}", context.Request.Path);
                }
                // 遍历所有内部异常，取第一个能匹配到友好消息的异常
                ex = flattened.InnerExceptions.FirstOrDefault(ie => FriendlyMessageMap.Keys.Any(m => m(ie))) ?? ex;
            }
