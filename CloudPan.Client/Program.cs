...
public static class Program
{
    ...
    public static async Task Main(string[] args)
    {
        ...
        // 命令行参数优先于保存的配置
        if (string.IsNullOrEmpty(ServerUrl) || ServerUrl.StartsWith("http://localhost"))
        {
            ServerUrl = clientConfig.ServerUrl;
        }

        if (string.IsNullOrEmpty(SyncRoot) || SyncRoot.StartsWith(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "CloudPan")))
        {
        ...
        try
        {
            ...
            // 商不支持则静默跳过（见 catch 块）。
            string? result = checkDb.Database.SqlQueryRaw<string>("PRAGMA quick_check;").FirstOrDefault();
            if (result != "ok")
            {
        ...
        bool connected = await apiClient.HealthCheckAsync(healthCts.Token);
        ...
        else
        {
            ...
            try
            {
                ...
                using HttpClient http = new HttpClient { BaseAddress = new Uri(ServerUrl.TrimEnd('/')) };
                http.Timeout = TimeSpan.FromSeconds(5);
                var versionResp = await http.GetAsync("/api/version", versionCts.Token);
                if (versionResp.IsSuccessStatusCode)
                {
                    string versionJson = await versionResp.Content.ReadAsStringAsync(versionCts.Token);
