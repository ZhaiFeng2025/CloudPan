...
public class ServerStatusService : IServerStatusService
{
    ...
    /// <inheritdoc />
    public async Task<List<AdminFileItem>> GetFilesAsync(string? path, int limit)
    {
        ...
        var items = await query
            ...
            .Select(f => new AdminFileItem(f.Path, f.Type, f.CurrentHash, f.CurrentSize, f.Version, f.State, f.LastModified))
            .ToListAsync();
        return items;
    }

    /// <inheritdoc />
    public async Task<List<AdminDeviceItem>> GetDevicesAsync()
    {
        await using var db = await _dbFactory.CreateDbContextAsync();
        var items = await db.Devices
            .OrderByDescending(d => d.LastSeen)
            .Select(d => new AdminDeviceItem(d.Id, d.Name, d.Person, d.LastSeen, d.Online, d.RegisteredAt))
            .ToListAsync();
        return items;
    }

    /// <inheritdoc />
    public async Task<List<AdminLogItem>> GetLogsAsync(int limit)
    {
        await using var db = await _dbFactory.CreateDbContextAsync();
        var items = await db.SyncLogs
            .OrderByDescending(l => l.CreatedAt)
            .Take(Math.Min(limit, 500))
            .Select(l => new AdminLogItem(l.Id, l.FilePath, l.Operation, l.DeviceId, l.Result, l.Details, l.CreatedAt))
            .ToListAsync();
        return items;
    }
    ...
    public async Task<AdminStatsResponse> GetStatsAsync()
    {
        ...
        return new AdminStatsResponse(fileCount, deviceCount, onlineCount, logCount);
    }
