...
public class ServerInstaller : Form
{
    ...
    private async Task InstallAsync()
    {
        ...
        try
        {
            ...
            await Task.Run(() => RunCmd("sc stop CloudPanServer"));
            await Task.Run(() => RunCmd("sc delete CloudPanServer"));
            ...
            // 对路径做 cmd 转义——防止路径中含 & ^ \" 等特殊字符导致命令注入
            string safeSyncDir = EscapeCmdArg(syncDir);
            bool createOk = await Task.Run(() =>
                RunCmd($"sc create CloudPanServer binPath= \"\\\"{exePath}\\\" --SyncRoot {safeSyncDir}\" DisplayName= \"CloudPan 文件同步服务\" start= auto"));
            if (!createOk)
            {
            ...
            await Task.Run(() => RunCmd("sc description CloudPanServer \"CloudPan File Sync Service\""));

            // M-04: 崩溃自动恢复
            await Task.Run(() =>
                RunCmd("sc failure CloudPanServer reset=86400 actions=restart/5000/restart/10000/restart/60000"));
            ...
            bool startOk = await Task.Run(() => RunCmd("sc start CloudPanServer"));
            ...
            await Task.Run(() => RunCmd("netsh advfirewall firewall delete rule name=\"CloudPan\""));
            await Task.Run(() =>
                RunCmd($"netsh advfirewall firewall add rule name=\"CloudPan\" dir=in action=allow protocol=TCP localport={SpecPorts.HttpPort}"));
    ...
    /// <summary>
    /// 对传给 cmd.exe 的参数做 shell 转义——防止路径中含 &amp; ^ \" 等特殊字符导致命令注入。
    /// 将参数用双引号包裹，内部双引号转义为 \"。
    /// </summary>
    ...
    /// <summary>
    /// 执行命令并返回是否成功（exit code == 0）。
    /// 失败时的错误详情会通过 LastRunCmdError 存储，供调用方在状态文字中展示。
    /// </summary>
    ...
    private static bool RunCmd(string command)
    {
        ...
        try
        {
            ProcessStartInfo psi = new ProcessStartInfo("cmd.exe", "/c " + command)
            {
                ...
                // 不重定向 stdout，仅捕获 stderr，避免管道缓冲死锁
            ...
            // 先等待进程退出（带超时），进程终止后再读取 stderr，保证不会死锁
            if (!p.WaitForExit(30000))
            {
                p.Kill();
                try { p.WaitForExit(1000); } catch { /* 忽略 kill 后的异常 */ }
    ...
    /// <summary>获取最近一次 RunCmd 调用的错误详情。</summary>
