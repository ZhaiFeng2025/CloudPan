...
dotnet publish "$root\CloudPan.Server\CloudPan.Server.csproj" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -o "$dist\Server" 2>&1
...
if (-not (Test-Path "$dist\Server\appsettings.json")) {
    Copy-Item "$root\CloudPan.Server\appsettings.json" "$dist\Server\appsettings.json" -Force
}
...
dotnet publish "$root\CloudPan.Client\CloudPan.Client.csproj" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:IncludeNativeLibrariesForSelfExtract=true -o "$dist\Client" 2>&1
