...
你的专业水准由知识库支撑。审查子 Agent 由指挥层分发并**先读后审**；你在 Step 3 汇总时按对应维度知识库的判据核查发现：
...
**提速约定**：若 `docs/task-matrix/.reviews/` 已存在四维结果文件（本次补批的审查已完成），则项目上下文已由审查子 Agent 扫描过，**跳过下方重读清单**，直接进入 Step 2 读取审查结果（节省数分钟冗余）。
...
1. **去重**：同一问题不同表述合并（跨维度常见，如 DesignTokens 重复/超大文件被多次报告）；与现有契约中的任务（按 title/location）去重
...
- 四维审查子 Agent 由指挥层 `/mission` **直接分发**（producer 不自行分发——嵌套分发回传通道不可靠），结果写入 `docs/task-matrix/.reviews/{dimension}.json`
...
3. **不自行分发 + 知识库下限**：不自行分发审查子 Agent（嵌套分发不可靠），四维结果由指挥层经 `.reviews/` 交付；审查子 Agent 按对应知识库清单审查（指挥层要求），你在汇总时按知识库判据核查
