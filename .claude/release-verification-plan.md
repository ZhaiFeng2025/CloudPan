...
$csprojVer = (Select-String -Path CloudPan.Client.UI/CloudPan.Client.UI.csproj -Pattern '<Version>(.*)</Version>').Matches.Groups[1].Value
$gitTag = git describe --tags --abbrev=0 2>$null
$gitTag = $gitTag -replace '^v', ''

Write-Host "shared-spec.json:  $specVer"
Write-Host "Client.UI.csproj: $csprojVer"
...
```powershell
# ========== 1. 硬编码端口扫描 ==========
$portHits = Select-String -Path "CloudPan.Server.Host\**\*.cs", "CloudPan.Client.Core\**\*.cs" `
...
$consoleHits = Select-String -Path "CloudPan.Server.Host\**\*.cs", "CloudPan.Client.Core\**\*.cs" `
    -Pattern 'Console\.(WriteLine|Error)\(' |
    Where-Object { $_.Path -notmatch 'Generated|obj|bin' }
# 过滤合法的：Server.Host Program.cs 中 Serilog 初始化前的 ShowError / 启动错误处理
$illegal = $consoleHits | Where-Object {
    $_.Path -notmatch 'CloudPan\.Server\.Host\\Program\.cs'
}
if ($illegal) {
...
|------|-----|------------|:---:|
| CloudPan.Contract | net8.0 | Library | ✅ |
| CloudPan.Infrastructure | net8.0-windows | Library | ✅ |
| CloudPan.Server.Core | net8.0-windows | Library | ✅ |
| CloudPan.Server.UI | net8.0-windows | WinExe | ✅ |
| CloudPan.Server.Host | net8.0-windows | WinExe | ✅ |
| CloudPan.Client.Core | net8.0-windows | Library | ✅ |
| CloudPan.Client.UI | net8.0-windows | WinExe | ✅ |
...
Copy-Item -Recurse CloudPan.Client.UI/bin/Release/net8.0-windows/ $env:TEMP/build-pass1/
...
$diff = Compare-Object `
    ...
    (Get-ChildItem CloudPan.Client.UI/bin/Release/net8.0-windows/ -Recurse -File | Select-Object -ExpandProperty Name | Sort-Object)
if ($diff) {
...
$pdb2 = (Get-ChildItem CloudPan.Client.UI/bin/Release/net8.0-windows/ -Recurse -Filter "*.pdb").Count
if ($pdb1 -ne $pdb2) {
...
$serverRefs = Select-String -Path CloudPan.Server.Host/CloudPan.Server.Host.csproj -Pattern 'ProjectReference'
$clientRefs = Select-String -Path CloudPan.Client.UI/CloudPan.Client.UI.csproj -Pattern 'ProjectReference'
$sharedRefs = Select-String -Path CloudPan.Contract/CloudPan.Contract.csproj -Pattern 'ProjectReference'
...
```powershell
$violations = @()
foreach ($proj in @("CloudPan.Server.Host", "CloudPan.Server.Core", "CloudPan.Server.UI", "CloudPan.Client.Core", "CloudPan.Client.UI", "CloudPan.Contract", "CloudPan.Infrastructure", "CloudPan.Tests")) {
    $prefix = $proj -replace '\.', '\.'
...
|------|-----------|-------------|------|------|
| CloudPan.Server.Core/ | 75% | 65% | 🔴 | 核心业务逻辑 |
| CloudPan.Server.Host/Middleware/ | 85% | 75% | 🔴 | 请求路径必经 |
| CloudPan.Server.Host/Controllers/ | 65% | 55% | 🔴 | 控制器层 |
| CloudPan.Client.Core/SyncEngine.cs | 60% | 50% | 🟡 | 文件系统依赖，难完全模拟 |
| CloudPan.Client.Core/FileWatcherService.cs | 55% | 45% | 🟡 | 依赖真实文件系统事件 |
| CloudPan.Contract/ | 70% | 60% | 🔴 | 共享类型 |
| **全局** | **65%** | **55%** | 🔴 | v1.0 最低门槛 |
...
```powershell
# 快速检查（代替完整浸泡测试）
$proc = Start-Process -FilePath "dotnet" -ArgumentList "run --project CloudPan.Server.Host -c Release" -PassThru
...
```powershell
$secretHits = Select-String -Path "CloudPan.Server.Host\**\*.cs", "CloudPan.Client.Core\**\*.cs" `
...
$exposedCloudpan = Select-String -Path "CloudPan.Server.Host\Controllers\**\*.cs" `
    -Pattern '\.cloudpan' |
...
foreach ($rid in $rids) {
    ...
    # 服务端
    dotnet publish CloudPan.Server.Host -c Release `
    ...
    # 客户端
    dotnet publish CloudPan.Client.UI -c Release `
...
```powershell
# 列出所有第三方 NuGet 包及许可证
dotnet list CloudPan.Client.UI package --include-transitive 2>&1 | Tee-Object nuget-packages.log
dotnet list CloudPan.Server.Host package --include-transitive 2>&1 | Tee-Object -Append nuget-packages.log
...
Check "Stage 1.2: 版本号一致性" {
    ...
    $csprojMatch = Select-String -Path CloudPan.Client.UI/CloudPan.Client.UI.csproj -Pattern '<Version>(.*)</Version>'
...
Check "Stage 9.1: Secret 泄露扫描" {
    $hits = Select-String -Path "CloudPan.Server.Host\**\*.cs", "CloudPan.Client.Core\**\*.cs" `
...
if (-not $SkipPublish) {
    Check "Stage 10: win-x64 发布" {
        dotnet publish CloudPan.Server.Host -c Release -o publish/win-x64/server/ `
        ...
        dotnet publish CloudPan.Client.UI -c Release -o publish/win-x64/client/ `
            --self-contained true -p:PublishSingleFile=true -p:RuntimeIdentifier=win-x64 2>&1 | Out-Null
