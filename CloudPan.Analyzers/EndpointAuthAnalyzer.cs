...
public sealed class EndpointAuthAnalyzer : DiagnosticAnalyzer
{
    ...
    private static void AnalyzeMethod(SyntaxNodeAnalysisContext context)
    {
        ...
        List<EndpointActionInfo> actions = EndpointRouteHelper.GetEndpoints(context.SemanticModel, method);
        if (actions.Count == 0)
        {
            return;
        }

        AttributeSyntax? authAttribute = FindEndpointAuth(context.SemanticModel, method);
    ...
    /// <summary>
    /// 方法特性优先，其次类级别特性（继承）。partial 类可能把类级特性声明在其它部分，
    /// 本文件声明上未找到时，用语义模型取合并后的类型符号，跨 partial 文件也能解析。
    /// </summary>
    private static AttributeSyntax? FindEndpointAuth(SemanticModel semanticModel, MethodDeclarationSyntax method)
    {
        ...
        AttributeSyntax? classLevel = EndpointRouteHelper.FindAttribute(classDecl.AttributeLists, "EndpointAuth");
        ...
        {
            if (attr.AttributeClass?.Name is "EndpointAuth" or "EndpointAuthAttribute"
                && attr.ApplicationSyntaxReference is not null)
            {
                return attr.ApplicationSyntaxReference.GetSyntax() as AttributeSyntax;
            }
        }
        return null;
    }
