...
internal static class EndpointRouteHelper
{
    ...
    public static List<EndpointActionInfo> GetEndpoints(SemanticModel semanticModel, MethodDeclarationSyntax method)
    {
        ...
        string? classRoute = GetClassRoute(semanticModel, classDecl);
    ...
    /// <summary>
    /// 解析类级 [Route] 模板。partial 类可能在其它部分声明 [Route]，
    /// 本文件声明上未找到时，用语义模型取合并后的类型符号，跨 partial 文件也能解析。
    /// </summary>
    private static string? GetClassRoute(SemanticModel semanticModel, ClassDeclarationSyntax classDecl)
    {
        string? local = GetFirstStringArgument(FindAttribute(classDecl.AttributeLists, "Route"));
